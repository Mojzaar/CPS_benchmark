function [ratio, N, A, exTimeAverage, totalTime] = HPSTL(b_u,dSingnificanceLevel,delta)
tic1 = tic;
% 
...
while sigLevel > dSingnificanceLevel
    ...
    for sampling = 1 : 1 % sampling from the model
        simOut  = sim('narmaSim','SaveOutput','on','OutputSaveName','oni');
        ONI = simOut.oni(end);
    end
    exTime(1,cnt)= toc; %consumed time for sampling
    %
    s(cnt) = (ONI < delta);% inteded specification 
    Nk = length(s);
    T = sum(s);
    if T / Nk < b_u
        z = 1; % interval [0 b]
